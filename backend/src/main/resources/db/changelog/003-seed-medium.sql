...
-- Opt-in via context=seed-medium; app runtime and codegen exclude it.
-- Mutually exclusive with seed-large (different contexts; never both in one run).
-- Runs before indexes (004) so the bulk insert hits an unindexed table — same
-- ordering as the seed-large flow.

--changeset phase6:003-seed-medium context:seed-medium runInTransaction:false
SELECT setseed(0.42);
